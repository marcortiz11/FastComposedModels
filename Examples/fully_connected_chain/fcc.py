...
def argument_parse(argv):
    ...
    parser.add_argument("--step_th", default=0.2, type=float, help="Threshold step, default=0.1")
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        out_dir = os.path.join("./results/", dataset, pid)
        data_path = "../../Data/"
        ...
        for model in models:
            ...
            R_models[model] = eval.evaluate(sys, c.id, phases=["test"]).test

        io.save_pickle(os.path.join(out_dir, "models"), R_models)
        front = paretto.get_front_time_accuracy(R_models)
            ...
            for th0 in np.arange(0, 1+step_th, step_th):
                for th1 in np.arange((th0+step_th)*fc, fc + step_th, step_th):
                    ...
                    for ic1 in range(ic0+1, len(models)):
                        ...
                        for th2 in np.arange(0, 1+step_th, step_th):
                            ...
                            for ic2 in range(ic1+1, len(models)):
                                ...
                                # Evaluate
                                id = "system_%s_(%f,%f)_%s_(%f)_%s" % (c0.split('/')[-1], th0, th1, c1.split('/')[-1], th2, c2.split('/')[-1])
                                results = eval.evaluate(sys, "c0")
                                records[id] = results.test
                                print(id)
            io.save_pickle(os.path.join(out_dir, "R"+c0.split('/')[-1]), records)
            records = {}
