...
def argument_parse(argv):
    ...
    parser.add_argument("--k", default=4, type=int, help="Tournament size")
    ...
    parser.add_argument("--comment", default="Auto_th_Fitness_normalize", type=str, help="")
    parser.add_argument("--experiment", default="bagging_boosting_of_chains_GA_2")
...
def mutation_operation(P, fit_vals):
    ...
    if operation == 0:
        ...
        om.extend_merged_chain_automatic_threshold(new_p, c_id_extend, c_id_new, args.a, c_file_new=c_file_new)
    ...
    if operation == 1:
        ...
        om.replace_classifier_merger_automatic_threshold(new_p, c_id_existing, c_id_new, args.a, c_file=c_file_new)
        new_p.set_sysid(utils.generate_system_id(new_p))
        offspring.append(new_p)
...
def evaluate_population(P, phases=['test', 'val']):
    ...
    if args.cores:
        assert args.device == 'none', "ERROR: Code does not evaluate the chain ensemble on PyTorch w\ CPU"
...
if __name__ == "__main__":
    ...
    fit = fit_fun.f2_time_param_penalization(R, args.a)
    P_all = []
    ...
    while iteration < args.iterations:
        ...
        fit_offspring = fit_fun.f2_time_param_penalization(R_offspring, args.a)
        # Selection
